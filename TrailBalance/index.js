      ...
      if (err) {
        ...
        data.forEach(v => {
          ...
          v.recordArr.forEach(i => {
            ...
            allArr.push({
              parent_code: v.party.company_code,
              _id: v.party._id,
              generated_on: v.party.generatedOn,
              record_no: v.record_no,
              data: i,
              party_name: v.party.head_of_ac,
              date: v.generatedOn,
              balance: startingBalance,
              op_bal: v.party.op_bal ? v.party.op_bal : 0,
              code: i.payment_mode.company_code,
            ...
            if (err) {
              ...
              data.forEach(v => {
                v.recordArr.forEach(i => {
                  ...
                  startingBalance += parseInt(i.amount);
                  allArr.push({
                    parent_code: v.payment_mode.company_code,
                    _id: v.payment_mode._id,
                    generated_on: v.payment_mode.generatedOn,
                    record_no: v.record_no,
                    data: i,
                    party_name: v.payment_mode.head_of_ac,
                    date: v.generatedOn,
                    balance: startingBalance,
                    op_bal: v.payment_mode.op_bal ? v.payment_mode.op_bal : 0,
                    code: i.party.company_code,
                    record_type: i.record_type
                  });
                });
              });
                  ...
                  if (err) {
                    ...
                    data.forEach(v => {
                      v.recordArr.forEach(i => {
                        ...
                        allArr.push({
                          parent_code: i.particulars.company_code,
                          _id: i._id,
                          generated_on: i.generatedOn,
                          record_no: v.record_no,
                          data: i,
                          party_name: i.particulars.head_of_ac,
                          date: v.generatedOn,
                          balance: startingBalance,
                          op_bal: i.particulars.op_bal
                            ? i.particulars.op_bal
                            : 0,
                          code: i.particulars.company_code,
                          record_type: i.record_type
                        });
                      });
                    });
                          ...
                          fun = (name, code) => {
                            accArr.map((v, i) => {
                              bankArr.map(b => {
                                comArr.map(c => {
                                  if (
                                    b.bank_code === code.slice(0, 5) &&
                                    v.d_code === code.slice(0, 3)
                                  ) {
